...
for multi-threading in the `betterChromVAR()` function.
...
If data is on disk, rather than in memory (and you want it to stay that way), 
make sure to use this procedure. In recent versions, `getExpectation()` should
use chunk-based processing. However it is quite likely that better performance
for this step would be obtained using for instance using the 
`aggregateAcrossCells()` function of the `r Biocpkg("scrapper")` package.
